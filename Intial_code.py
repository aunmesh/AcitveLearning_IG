...
batch_size = 25
k =10
gradient = np.zeros(100000, dtype=float).reshape(10,10000)
c_i = np.zeros(100000, dtype=float).reshape(10,10000)
class_order = np.arange(100000).reshape(10,10000)
...
def complete_dict(grad, output,image_index, count, batch_size):
    ...
    for batch_number in range(batch_size): 
        mul = torch.norm(grad[batch_number], 2)
        ind = image_index[batch_number]
        gradient[count][ind] = mul.detach().cpu().numpy()         
        c_i[count][ind] = sorting[batch_number][count].detach().cpu().numpy()         
        class_order[count][ind] = indices[batch_number][count]
...
for i, data in enumerate(test_data_loader):
    ...
    for index in indices:
        ...
        grad_input = torch.autograd.grad(loss, inputs,retain_graph = True, allow_unused=True)
